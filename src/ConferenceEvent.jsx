...
import { incrementAddOnQuantity, decrementAddOnQuantity} from "./addOnsSlice";
...
const ConferenceEvent = () => {
    ...
    const getItemsFromTotalCost = () => {
        const items = [];
        items.push(...venueItems.filter(item => item.quantity > 0).map(item => ({ ...item, type: "venue" })));
        addonItems.forEach((item) => {
            if (
              item.quantity > 0 &&
              !items.some((i) => i.name === item.name && i.type === "addon")
            ) {
    ...
    const calculateTotalCost = (section) => {
        ...
        if (section === "venue") {
          venueItems.forEach((item) => {
            ...
            totalCost = mealsItems.reduce((acc, item) => acc + numberOfPeople*item.cost,0);
        }
        return totalCost;
      };
